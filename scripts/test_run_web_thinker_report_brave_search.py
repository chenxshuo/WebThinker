...
async def process_single_sequence(
    ...
    # Check if search_plan exists in dataset and is not empty
    if 'search_plan' in seq['item'] and seq['item']['search_plan']:
        search_plan = seq['item']['search_plan']
        print(f"---Using search plan from dataset:---\n{search_plan}")
    else:
        # Generate new search plan if not available in dataset
    ...
    # Store search plan in sequence
    seq['search_plan'] = search_plan
    ...
    while not seq['finished']:
        ...
        if response.rstrip().endswith(END_WRITE_SECTION):
            ...
            # Generate search intent and get user input
            print(f"\nCurrent search query: {search_query}")
            _, model_intent = await generate_response(
                client=aux_client,
                model_name=args.aux_model_name,
            ...
            # Store model generated intent first
            search_intent_entry = {
                "search_query": search_query,
                "model_intent": model_intent,
                "user_intent": ""
            }
            seq['search_intents'].append(search_intent_entry)
            
            # Get user input for search intent
            print(f"Model generated search intent: {model_intent}")
            print("Please enter your search intent (press Enter to use model's intent):")
            user_intent = input().strip()
            
            # Update search intent with user input
            if user_intent:
                search_intent = user_intent
                search_intent_entry['user_intent'] = search_intent
            else:
                search_intent = model_intent
                search_intent_entry['user_intent'] = model_intent
...
async def main_async():
    ...
    if args.dataset_name == 'strongreject':
        ...
        output_dir = f'./outputs/test.{model_short_name}.webthinker'
    ...
    for item in filtered_data:
        active_sequences.append({
            ...
            'search_intents': [],  # Add list to store search intents
        })
    ...
    if args.eval:
        ...
        for item, seq in zip(filtered_data, completed_sequences):
            ...
            item['SearchPlan'] = seq.get('search_plan', '')  # Add search plan
            item['SearchIntents'] = seq.get('search_intents', [])  # Add search intents
