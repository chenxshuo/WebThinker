...
async def main_async():
    ...
    # Create markdown directory with timestamp
    t = time.localtime()
    random_num = str(random.randint(0, 99)).zfill(2)
    markdown_dir = os.path.join(
        output_dir,
        f'markdown.{args.split}.{t.tm_mon}.{t.tm_mday},{t.tm_hour}:{t.tm_min}.{random_num}'
    )
    os.makedirs(markdown_dir, exist_ok=True)

    def save_markdown(seq, idx):
        """Save markdown file for a completed sequence"""
        if seq.get('article', '').strip():
            item_id = seq['item'].get('id', idx + 1)
    ...
    try:
        ...
        # Run all sequences concurrently with progress bar and real-time markdown saving
        with tqdm(total=len(tasks)) as pbar:
            async def track_progress(task, idx):
                result = await task
                save_markdown(result, idx)  # 实时保存 markdown 文件
                pbar.update(1)
                return result
        ...
        for item, seq in zip(filtered_data, completed_sequences):
            ...
            item['WebExplorer'] = seq['web_explorer']
