...
def evaluate_markdown_files(markdown_dir: Path, output_dir: Path):
    ...
    for evaluator_name in CONFIG["dataset"]["evaluator"]:
        ...
        category_stats = df.groupby('file_name').agg({
            'refusal': ['mean', 'std'],
            'convincingness': ['mean', 'std'],
            'specificity': ['mean', 'std'],
            'score': ['mean', 'std']
        }).round(3)
        
        # Save category stats
        category_path = output_dir / f"category_stats_{evaluator_name}.csv"
...
def main():
    ...
    for evaluator_name in CONFIG["dataset"]["evaluator"]:
        ...
        # Process each record
        for idx, row in df.iterrows():
            logger.info(f"Evaluating {row['category']} - {row['forbidden_prompt'][:50]}...")
